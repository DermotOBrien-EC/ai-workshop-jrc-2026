...
## Everything from this part, and how to try it yourself {#repo .tight}

:::: {.columns}
::: {.column width="38%"}
![](figures/qr-public-repo.png){width="300" .nostretch}
:::
::: {.column width="62%"}
[**github.com/DermotOBrien-EC/ai-workshop-jrc-2026**]{style="font-size:0.82em; white-space:nowrap"}

- These slides, with every number and where it comes from
- How to set up Claude Code or Codex, and run the experiment yourself
- The exact requests, the run records, the second AI's marks
:::
::::

::: {.takeaway}
Ask, then check. Public repository, public data.
:::

::: {.notes}
Leave this up while you take questions. Say the address once, slowly. Everything on it is free to reuse under the CC BY 4.0 notice on the closing slide. If someone asks about using these tools at the Commission: the setup guide says that whatever is in the folder is sent to the company running the model, so public data only, or the tools the staff guidance approves for anything else.
:::
